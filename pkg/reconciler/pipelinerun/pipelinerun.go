...
func (c *Reconciler) runNextSchedulableTask(ctx context.Context, pr *v1.PipelineRun, pipelineRunFacts *resources.PipelineRunFacts) error {
	...
	for _, rpt := range nextRpts {
		...
		err := resources.CheckMissingResultReferences(ctx, pipelineRunFacts.State, rpt)
		if err != nil {
	...
	if len(fNextRpts) != 0 {
		...
		for _, rpt := range fNextRpts {
			resolvedResultRefs, _, err := resources.ResolveResultRef(ctx, pipelineRunFacts.State, rpt)
			if err != nil {
	...
	for _, rpt := range nextRpts {
		...
		if rpt == nil || rpt.Skip(ctx, pipelineRunFacts).IsSkipped || rpt.IsFinallySkipped(pipelineRunFacts).IsSkipped {
			continue
		}

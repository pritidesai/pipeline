...
func (c *Reconciler) reconcile(ctx context.Context, pr *v1beta1.PipelineRun) error {
	...
	for _, t := range pipelineSpec.Tasks {
		if !inList(pr.Status.State.DAGTasks, t.Name) {
			pr.Status.State.DAGTasks = append(pr.Status.State.DAGTasks, t.Name)
		}
	}
	for _, t := range pipelineSpec.Finally {
		if !inList(pr.Status.State.FinalTasks, t.Name) {
			pr.Status.State.FinalTasks = append(pr.Status.State.FinalTasks, t.Name)
		}
	}

	spew.Dump("PipelineRun State Default")
	spew.Dump(pr.Status.State)
	spew.Dump("PipelineRun State Default")
	...
	nextRprts := pipelineState.GetNextTasks(candidateTasks, pr.Status.State)
	...
	pr.Status.SetPipelineTaskState(pipelineState.UpdatePipelineTaskState(pr, d))

...
func (c *Reconciler) getPipelineFunc(tr *v1alpha1.PipelineRun) resources.GetPipeline {
	var gtFunc resources.GetPipeline
	gtFunc = func(name string) (v1alpha1.PipelineInterface, error) {
...
*/
	...
	providedResources, err := resources.GetResourcesFromBindings(pipelineSpec, pr)
	if err != nil {
		// This Run has failed, so we need to mark it as failed and stop reconciling it
		pr.Status.SetCondition(&apis.Condition{
			Type:   apis.ConditionSucceeded,

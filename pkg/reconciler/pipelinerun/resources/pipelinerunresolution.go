...
func (state *PipelineRunState) createRunOnConflictedTaskRun(t *ResolvedPipelineRunTask) *v1alpha1.TaskRun {
	tr := &v1alpha1.TaskRun{
		ObjectMeta: metav1.ObjectMeta{
			Name: t.TaskRunName,
		},
	}
	tr.Status.SetCondition(
		&apis.Condition{
			Type:    apis.ConditionSucceeded,
			Status:  corev1.ConditionFalse,
			Reason:  ReasonSkippedAsStateConflicted,
			Message: fmt.Sprintf("RunOn states conflicted for Task %s in PipelineRun", t.PipelineTask.Name),
		})
	return tr
}
*/
...
func (state *PipelineRunState) CompletedPipelineTaskNames(pr *v1alpha1.PipelineRun) []string {
	spew.Dump("I am inside CompletedPipelineTaskNames")
	//spew.Dump("**** pipelineRunState ****")
	//spew.Dump(state)
	//spew.Dump("**** pipelineRunState ****")
	done := []string{}
	for _, t := range *state {
		if t.TaskRun != nil {
			c := t.TaskRun.Status.GetCondition(apis.ConditionSucceeded)
			if c.IsTrue() {
				spew.Dump("task was successful and appending it now")
				spew.Dump(t.PipelineTask.Name)
				done = append(done, t.PipelineTask.Name)
			} else if c.IsFalse() {
				if c.Reason == ReasonFailed && t.IsFailurePermitted {
					spew.Dump("task failed but appending it now")
					spew.Dump(t.PipelineTask.Name)
					spew.Dump(c.Reason)
					done = append(done, t.PipelineTask.Name)
				}
			...
			if pr.Status.PipelineRunStatusFields.TaskRuns != nil {
				//spew.Dump("START: pipelinerunstatus taskruns is empty looking into pipelinerun now")
				//spew.Dump(pr)
				//spew.Dump("END: pipelinerunstatus taskruns is empty looking into pipelinerun now")
				for _, s := range pr.Status.TaskRuns {
					if s.Status != nil {
						c := s.Status.GetCondition(apis.ConditionSucceeded)
						if c != nil {
							if t.PipelineTask.Name == s.PipelineTaskName && c.IsFalse() && c.Reason == ReasonSkippedAsStateConflicted {
								spew.Dump("appending task while iterating over pipelinerun")
								spew.Dump(s.PipelineTaskName)
...
func GetPipelineConditionStatus(pr *v1alpha1.PipelineRun, state PipelineRunState, logger *zap.SugaredLogger, dag *dag.Graph) *apis.Condition {
	//spew.Dump("Calling PipelineConditionStatus")
	//spew.Dump("***** pr from PipelineConditionStatus *****")
	//spew.Dump(pr)
	//spew.Dump("***** pr from PipelineConditionStatus *****")
	...
	//for _, tr := range pr.Status.PipelineRunStatusFields.TaskRuns {
	//	spew.Dump("I am inside for loop, iterating over pr.Status.TaskRuns")
	//	spew.Dump("*** tr ****")
	//	spew.Dump(tr)
	//	spew.Dump("*** tr ****")
	//	if tr.Status != nil && tr.Status.GetCondition(apis.ConditionSucceeded) != nil && tr.Status.GetCondition(apis.ConditionSucceeded).Reason == ReasonSkippedAsStateConflicted {
	//		successOrSkipTasks = append(successOrSkipTasks, tr.PipelineTaskName)
	//		skipTasks++
	//	}
	//}

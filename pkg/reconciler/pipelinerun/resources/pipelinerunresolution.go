...
const (
	...
	// ReasonSkippedRunOnStatusMismatch indicates that a task had specified a certain state of a parent in runOn
	// but didnt match with the actual state of that parent
	ReasonSkippedRunOnStatusMismatch = "SkippedRunOnStatusMismatch"
)
...
type ResolvedPipelineRunTask struct {
	...
	// SkipTaskRun is set to true for a task which will be skipped since runOn states does not match with the states of parent task
	SkipTaskRun bool
}
...
// isSkippedRunOnStatusMismatch return true only if the taskRun has failed with reason SkippedRunOnStatusMismatch
func (t ResolvedPipelineRunTask) isSkippedRunOnStatusMismatch() bool {
	return t.SkipTaskRun
}
...
func (state PipelineRunState) getNextTasksWithoutTaskRuns(t *ResolvedPipelineRunTask) []*ResolvedPipelineRunTask {
	...
	if len(t.PipelineTask.RunAfter) == 0 {
		...
		if state.isRunAfterSkipped(t.PipelineTask.RunAfter) {
			// one of the tasks specified in runAfter was marked as Conflicted therefore
			// mark this task as conflicted using "ReasonSkippedRunOnStatusMismatch" as well
			t.SkipTaskRun = true
			tasks = append(tasks, t)
		} else {
			...
			if len(t.PipelineTask.RunOn) == 0 {
				...
				if state.validateRunOn(t.PipelineTask.RunOn) {
					...
					// this task will be marked as conflicted using "ReasonSkippedRunOnStatusMismatch"
					t.SkipTaskRun = true
					tasks = append(tasks, t)
				}
			}
...
func (state PipelineRunState) isRunAfterSkipped(runAfter []string) bool {
	for _, t := range state {
		for _, task := range runAfter {
			if t.PipelineTask.Name == task {
				return t.isSkippedRunOnStatusMismatch()
			}
		}
	}
	return false
		...
		if t.TaskRun != nil {
			...
			if c.IsTrue() {
				...
				if (c.Reason == ReasonFailed && t.IsFailurePermitted) ||
					c.Reason == ReasonSkippedRunOnStatusMismatch {
					done = append(done, t.PipelineTask.Name)
				}
			}
		} else if t.SkipTaskRun {
			done = append(done, t.PipelineTask.Name)
		}
	}
	return done
...
func ResolvePipelineRun(
	...
	for i := range tasks {
		...
		if err != nil {
			...
			if pipelineRun.Status.TaskRuns[rprt.TaskRunName] != nil {
				prts := pipelineRun.Status.TaskRuns[rprt.TaskRunName]
				if prts.SKipped {
					rprt.SkipTaskRun = true
				}
			}
		}
		if taskRun != nil {
...
func GetPipelineConditionStatus(pr *v1alpha1.PipelineRun, state PipelineRunState, logger *zap.SugaredLogger, dag *dag.Graph) *apis.Condition {
	...
	for _, rprt := range state {
		...
		if rprt.IsFailure() { //IsDone ensures we have crossed the retry limit
			...
			if !(rprt.IsFailurePermitted || rprt.isSkippedRunOnStatusMismatch()) {
	...
	for _, rprt := range state {
		...
		if rprt.isSkippedRunOnStatusMismatch() {
			successOrSkipTasks = append(successOrSkipTasks, rprt.PipelineTask.Name)
			skipTasks++
		}
	}

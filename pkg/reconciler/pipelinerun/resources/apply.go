			...
			for _, pt := range facts.State {
				if pt.PipelineTask.Name == pipelineTaskName {
					...
					if len(pt.ResultsCache) == 0 {
						cache := createResultsCacheMatrixedTaskRuns(pt)
						resultsCache = &cache
						pt.ResultsCache = *resultsCache
					}
					resultsLength = len(pt.ResultsCache[resultName])
					continue
				}
			}
			replacements["tasks."+pipelineTaskName+".matrix."+resultName+".length"] = strconv.Itoa(resultsLength)
		}
	}

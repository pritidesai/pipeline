...
func TestPipelineRunState_GetResultsFuncs(t *testing.T) {
	...
	if !cmp.Equal(expectedTaskResults, actualTaskResults) {
		d := cmp.Diff(expectedTaskResults, actualTaskResults)
		t.Errorf("Didn't get expected TaskRun results map: %s", diff.PrintWantGot(d))
	}

	actualRunResults := state.GetRunsResults()
	if !cmp.Equal(expectedRunResults, actualRunResults) {
		d := cmp.Diff(expectedRunResults, actualRunResults)

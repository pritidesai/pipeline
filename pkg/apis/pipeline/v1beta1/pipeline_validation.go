...
func (ps *PipelineSpec) Validate(ctx context.Context) (errs *apis.FieldError) {
	...
	errs = errs.Also(validateResultsFromMatrixedPipelineTasksConsumed(ps.Tasks, ps.Finally))
	return errs
}
...
// validateMatrixedPipelineTaskConsumed checks that any Matrixed Pipeline Task that the is becing consumed is consumed in
	...
	for _, expression := range expressions {
		...
		if taskConsumed.IsMatrixed() {
			if !strings.HasSuffix(expression, "[*]") {
				errs = errs.Also(apis.ErrGeneric(fmt.Sprintf("A matrixed pipelineTask can only be consumed in aggregate using [*] notation, but is currently being indexed %s", expression)))
			}
...
// validateResultsFromMatrixedPipelineTasksConsumed checks that any Matrixed Pipeline Task that the is becing consumed is consumed in
// aggregate [*] since consuming a singular result produced by a matrix is currently not supported.
...
func validateResultsFromMatrixedPipelineTasksConsumed(tasks []PipelineTask, finally []PipelineTask) (errs *apis.FieldError) {
	errs = errs.Also(validateTaskResultsFromMatrixedPipelineTasksConsumed(tasks))
	errs = errs.Also(validateTaskResultsFromMatrixedPipelineTasksConsumed(finally))
	return errs
}

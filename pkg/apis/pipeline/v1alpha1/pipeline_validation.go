...
func (ps *PipelineSpec) Validate(ctx context.Context) *apis.FieldError {
	...
	//(TODO - pipelinelevel finally) validation:
	// (1) validate final from for ps.Finally
	// (2) validate ps.Finally - no runAfter in final tasks
	// (3) validate ps.Finally - no conditions in final tasks
	return nil
}
